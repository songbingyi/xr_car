...
export class QrcodeInfoComponent implements OnInit {
        ...
        this.baseService.post('getMemberDetail', {})
            .subscribe(memberDetail => {
                if (memberDetail.status.succeed === '1') {
                    ...
                    this.shouldShowWarningBox = this.memberDetail.member_auth_info.identity_auth_status === '1';
                    // TODO
                    this.baseService.post('getMemberDetail', {})
                        .subscribe(memberDetail => {
                            if (memberDetail.status.succeed === '1') {
                                this.identityAuthStatus = memberDetail.data.member_auth_info.identity_auth_status === '1';
                            } else {
    ...
    save() {
        ...
        this.baseService.post("path", {
            name: this.qrcodeInfoForm.value.name,
            mobile: this.qrcodeInfoForm.value.phone,
            companyName: this.qrcodeInfoForm.value.companyName,
        })
            .subscribe(result => {
